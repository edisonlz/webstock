...
function trend_stock()
{
	...
	this.setdata=function(idata)
	{
	   ...
	   tctx.save();
       tctx.clearRect(0, 0, tcanvasWidth, tcanvasHeight);
	   alert("draw");
	   if(tdata<=5)
	   {
	     tctx.fillStyle =  'rgb(102,204,0)';
		 tctx.fillRect(2,0,145,tdef_w);
	     for (i=0;i<10-tdata;i++)
	     {
    		tctx.fillStyle = 'rgba(255,255,255,'+(i+1)/10+')';
    		for (j=0;j<2;j++)
			{
      			tctx.fillRect(i*14+1,j*tdef_w+1,14,17.5)
			}
         }
	   	tctx.save();
	   	tctx.fillStyle = 'rgb(255,0,0)';
  	   	tctx.fillRect(2,tdef_w,145,tdef_w);
	   	for (i=0;i<(tdata);i++)
	   	{
    		tctx.fillStyle = 'rgba(255,255,255,'+(i+1)/10+')';
    		for (j=0;j<2;j++)
	...
	function initial_trend()
	{
		...
		tcanvasWidth = ttarget.width();
        tcanvasHeight = ttarget.height();
        ttarget.html(""); // clear target
        //ttarget.css("position", "relative"); // for positioning labels and overlay
        if (tcanvasWidth <= 0 || tcanvasHeight <= 0)
                throw "Invalid dimensions for plot, width = " + tcanvasWidth + ", height = " + tcanvasHeight;
            // the canvas
        tcanvas = $('<canvas width="' + tcanvasWidth + '" height="' + tcanvasHeight + '"></canvas>').appendTo(ttarget).get(0);
        if ($.browser.msie) // excanvas hack
                tcanvas = window.G_vmlCanvasManager.initElement(tcanvas);
        tctx = tcanvas.getContext("2d");
		 alert("end--initial_trend");
	}
}
//*---------- canvas draw end----------*///
