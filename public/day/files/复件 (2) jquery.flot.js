            ...
            function plotPoints(data, radius, fill) {
				...
				for (var i = 0; i < data.length; ++i) {
                    ...
                    if (fill)
                        ctx.fill();
                    ctx.stroke();
					htm+='<b style="cursor:hand;position:absolute;left:'+(tHoz(x)+v_v)+'px;top:'+(tVert(y)+v_v)+'px;width=10px height=10px" onclick="t_load('+data[i][3]+')"><font style="font-size:12px;color:#993333;">'+data[i][2]+'</font></b>';
					if(v_v==10)
					{
						v_v=-10;
					}
