...
var url=location.search; 
 ...
 $(document).ready(function() { 
	...
	$("#stock_id").val(code);
	$.get("http://"+host+":"+port+"/stock/stock_view_all?code="+code, function(data){
	 ...
	 //------Stock info -------
	 setTimeout("stock_info_lets_go()",1500);
	 start_auto_completes();
	 load_financial_info();
	 register_ajax_comments();
	 //$("body").keydown( function() {$("#stock").val()} );

 });
 function stock_info_lets_go()
 {
	 stock_letsgo("http://rss.cnfol.com/1325.xml",$("#day_view_info"));
 }
 ///定时读取
 function timer_load()
 {
...
function start_auto_completes()
{
	...
	{
		$.get("http://"+host+":"+port+"/stock/stock_radar", function(data){
			 if(data.length<=0)
			 {	return;	}
			 var t=[];
...
//*------------stock info---------------*//
var base_uri="http://"+host+":"+port+"/stock/rss_loader?uri=";
...
{
	$.get("http://"+host+":"+port+"/stock/financial_loader?code="+code, function(data){
...
function stock_load_day()
{
	if(ttcode_show!=code)
	{
		$.get("http://"+host+":"+port+"/stock/stock_day?code="+code, function(data){
...
//------------commments--------
function register_ajax_comments()
{ 
	setTimeout("commemt_load_data()",3500);
}

function commemt_load_data()
{
	$("#commentText").load(decodeURI('http://"+host+":"+port+"/stock/comment_read?code='+code));
}
...
function send_comment() { 
		...
		var url_c="http://"+host+":"+port+"/stock/comment_add?name="+encodeURI($("#name").val())+"&code="+(code)+"&content="+encodeURI($("#comment_content").val());
		//$("#comment_content").val(url_c);
		$.post(url_c);
		register_ajax_comments();
		is_post=true;
		alert("评论成功！");
