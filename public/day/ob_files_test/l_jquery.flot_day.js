...
(function($) {
    function Plot(target_, data_, options_) {
        ...
        var options = {
            ...
            grid: {
                color: "#aa3333", // primary color used for outline and labels
                backgroundColor: null, // null for transparent, else color
                tickColor: 'rgb(100,100,100)', // color used for the ticks
        ...
        function fillInSeriesOptions() {
            ...
            for (i = 0; i < series.length; ++i) {
				...
				s.candles = $.extend(true, {}, options.candles, s.candles);
        ...
        function constructCanvas() {
			...
			{	
				canvasWidth=this.def_width;
				canvasWidth=this.def_height;
				//canvasWidth=665;
				//canvasWidth=665;
            ...
            if (options.grid.clickable)
                eventHolder.mousemove(onClick);
			else if(options.grid.zmoveable)
				eventHolder.mousemove(onMove_Data);
        }
        ...
        function prepareTickGeneration(axis, axisOptions) {
			...
			/*if(axisOptions.mode == "stock_time")
			{		
				var c=[];
				var timeNames = [" 9:30 "," 10:00 ", " 10:30 ", " 11:00 "," 11:30 "," 13:30 ", " 14:00 ", " 14:30 ", " 15:00 "];
				return timeNames.join("");
			}
            else */if (axisOptions.mode == "time") {
        ...
        function insertLabels() {
            ...
            var it=parseInt( yaxis.ticks.length/2);
            // do the y-axis
            for (i = 0; i < yaxis.ticks.length; ++i) {
                tick = yaxis.ticks[i];
				...
				if(i<it)
				{
					html += '<div style="position:absolute;top:' + (plotOffset.top + tVert(tick.v) - yLabelMaxHeight/2) + 'px;left:0;width:' + yLabelMaxWidth + 'px;text-align:right" class="tickLabel" ><font color="#009933">' + tick.label + "</font></div>";
				}
				else
				{
        ...
        function drawSeries(series) {
            if (series.lines.show || (!series.bars.show && !series.points.show && !series.candles.show))
                drawSeriesLines(series);
            if (series.bars.show)
                drawSeriesBars(series);
            if (series.points.show)
                drawSeriesPoints(series);
			if(series.candles.show)
				drawSeriesCandles(series);
        }
		...
		///New Add  Draw Candles --- go,go,go
		 function drawSeriesCandles(series) 
		 {
            function plotCandles(data) 
			{
                var htm="";
				var v_v=10;
				var day_span=canvasWidth/data.length;
				var red="rgb(255,0,0)";
				var green="rgb(0,255,10)";
				var color=red;
				var day_max,day_min,day_open,day_close;
				var day_index;
				var draw_day_value=0;
				var day_x=0;
				var max_w;
				var open_w;
				var vert_max;
				for (var i = 0; i < data.length; ++i) {
                    if (data[i] == null)
                        continue;
					day_max=data[i][1],day_min=data[i][2],day_open=data[i][3],day_close=data[i][4];
					day_index=data[i][0];
					//alert(day_open+","+day_close);
					if(parseFloat(day_open)>parseFloat(day_close))
					{
						color=green;
					}
					else
                    ...
                    if (day_index < xaxis.min || day_index > xaxis.max || day_max < yaxis.min || day_max > yaxis.max)
                        continue;
					//ctx.clearRect(0, 0, canvasWidth, canvasHeight);	
                    ctx.beginPath();
					ctx.fillStyle = color;
					vert_max=tVert(day_max);
					max_w=Math.abs(tVert(day_min)-vert_max);
					if(max_w==0)
						max_w=1;
				 	ctx.fillRect(tHoz(day_index),vert_max,1, max_w);
					ctx.fillStyle = color;
					draw_day_value=day_open;
					if(day_open<day_close)
					{
						draw_day_value=day_close;
					}
					if(i==0)
					{
						day_x=tHoz(day_index);
					}
					else
					{
						day_x=tHoz(day_index)-day_span/2;
					}
					open_w=Math.abs(tVert(day_open)-tVert(day_close));
					if(open_w==0)
						open_w=1;
					if(i==0 || i==data.length-1)
					{
						ctx.fillRect(day_x,tVert(draw_day_value),day_span/2,open_w);
					}
					else
					{
						ctx.fillRect(day_x,tVert(draw_day_value),day_span,open_w);
					}
                }
            }
            ctx.save();
            ...
            ctx.strokeStyle = series.color;
            plotCandles(series.data);
            ctx.restore();
        }
		///end New Add  Draw Candles --- go,go,go
            ...
            function plotPoints(data, radius, fill) {
				...
				for (var i = 0; i < data.length; ++i) {
                    ...
                    if (fill)
                        ctx.fill();
                    ctx.stroke();
					htm+='<b style="cursor:hand;position:absolute;left:'+(tHoz(x)+v_v)+'px;top:'+(tVert(y)+v_v)+'px;width=10px height=10px" onclick="load_data('+data[i][3]+')"><font style="font-size:12px;color:#993333;">'+data[i][2]+'</font></b>';
					if(v_v==10)
					{
						v_v=-10;
					}
        ...
        function onClick(e) {
		    ...
		    if(def_data[0][parseInt(pos.x)]!=null)
	       {
       		  pos.y=def_data[0][parseInt(pos.x)][1]+":"+def_data[0][parseInt(pos.x)][2];
	          target.trigger("plotclick",[pos]);
    	   }
        }
		
		function onMove_Data(e) {
            var offset=eventHolder.offset();
		    var pos=
		    {
		    };
		    pos.x=e.pageX-offset.left-plotOffset.left;
		    pos.x=xaxis.min+pos.x/hozScale;
			var day_span=canvasWidth/def_data.length;
			//alert(pos.x);
		    if(def_data[0][parseInt(pos.x)]!=null)
	       {
       		  pos.y=def_data[0][parseInt(pos.x)][1]+"-"+def_data[0][parseInt(pos.x)][2]+"-"+def_data[0][parseInt(pos.x)][3]+"-"+def_data[0][parseInt(pos.x)][4]+"-"+def_data[0][parseInt(pos.x)][5];
	          target.trigger("moveclick",[pos]);
    	   }
        }
